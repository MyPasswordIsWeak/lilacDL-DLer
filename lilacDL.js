...
const { readFileSync, mkdirSync, existsSync,appendFileSync, unlinkSync, openSync, closeSync, rmdirSync, writeFileSync, readdirSync } = require('fs');
...
// Download part
require('./downloadChain.js')(Files,links,basePath,fileTitle,flags,md5sum,false)
    .then(errors => {
        
        // Dump errors if wanted
        if(flags.dumpErr)
        	writeFileSync('./DumpedErrors.json',JSON.stringify(errors, null, 2));
        ...
        errorHandler.once('done', function() { 
            ...
            if(flags.joinFiles) {
                ...
                for(let i = 0; i < Files; ++i) {
                    ...
                    if(flags.debug) {
                        console.log(`Appended part the links[i] object ${links[i]}`);
                        console.log(`i = ${i}, links.length = ${links.length}`);
                        console.log(`tmpdirsize: ${readdirSync(basePath).length}`);
                        console.log(`i will at a maximum reach ${Files} - 1 (${Files - 1})`);
                        console.log('-----------------------------------------------------------');
                    } else {
